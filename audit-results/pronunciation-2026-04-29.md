...
**Voice:** Johanna (`v3V1d2rk6528UrLKRuy8`)  
**Model:** `eleven_multilingual_v2`  
**Settings:** stability=0.55, similarity=0.8, style=0.15, speed=0.95  
...
|---|---|---|---|---|---|
| 1 | Guten Tag, hier ist Lisa vom Fachwelt Verlag. | `fachwelt` / `verlag` | ... (vier Sekunden Pause) Guten Tag, hier ist Lisa vom Fachweltverlag. | **PASS** | `01_guten_tag_hier_ist_lisa_vom_fachwelt_verlag.mp3` |
...
| 4 | fachwelt-marketpleis punkt de | `marketpleis` / `marketplace` | Fachwelt Marketplace Punkte. | **PASS** | `04_fachwelt_marketpleis_punkt_de.mp3` |
| 5 | Marketpleis | `marketpleis` / `marketplace` | Marketplace. | **PASS** | `05_marketpleis.mp3` |
| 6 | Wir bauen einen Marketpleis für B2B-Hersteller. | `marketpleis` / `marketplace` | Wir bauen einen Marketplace für B2B Hersteller. | **PASS** | `06_wir_bauen_einen_marketpleis_für_b2b_hersteller.mp3` |
| 7 | Es geht um B2B. | `be zu be` / `be-zu-be` / `b2b` | Es geht B zu B. | **FAIL** | `07_es_geht_um_b2b.mp3` |
| 8 | Ich bin ein KI-Assistent vom Fachwelt Verlag. | `ki` / `ka i` / `ka-i` | Ich bin ein KAI Assistent vom Fachweltverlag. | **FAIL** | `08_ich_bin_ein_ki_assistent_vom_fachwelt_verlag.mp3` |
...
| 14 | Lead generieren. | `lied` / `lead` | Lied generieren. | **PASS** | `14_lead_generieren.mp3` |
| 15 | Hätten Sie kurz zwei Minuten? | `zwei minuten` | Hätten Sie kurz zwei Minuten? | **PASS** | `15_hätten_sie_kurz_zwei_minuten.mp3` |
| 16 | max punkt mustermann at firma minus beispiel punkt de | `mustermann` / `firma` | Max Punkt Mustermann at Firma minus Beispiel Punkt de. | **PASS** | `16_max_punkt_mustermann_at_firma_minus_beispiel_punkt_de.mp3` |
...
- `Es geht um B2B.`
  ...
  - Gehört: `Es geht B zu B.`
  - **Vorschlag:** PLS-Alias in Dict `IW2jbKiFNq8Nz0X4kscp` ergänzen, neue Version-ID in `src/agent.py` einsetzen. Edon's Decision — nicht automatisch.
- `Ich bin ein KI-Assistent vom Fachwelt Verlag.`
  - Erwartet: ['ki', 'ka i', 'ka-i']
  - Gehört: `Ich bin ein KAI Assistent vom Fachweltverlag.`
  - **Vorschlag:** PLS-Alias in Dict `IW2jbKiFNq8Nz0X4kscp` ergänzen, neue Version-ID in `src/agent.py` einsetzen. Edon's Decision — nicht automatisch.